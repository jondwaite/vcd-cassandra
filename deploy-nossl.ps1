...
- content: |
    ...
    cqlsh $($_.value) -u cassandra -p 'cassandra' -e "ALTER USER cassandra WITH PASSWORD '$($CassPass)';"
    rm -- `$0
  permissions: '0755'
...
Write-Host -ForegroundColor Yellow ("`nSince SSL is disabled you must also set 'cassandra.use.ssl' to 0 in VCD global.properties file on all cells")
